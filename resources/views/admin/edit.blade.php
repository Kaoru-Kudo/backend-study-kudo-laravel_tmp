...
<section>
    ...
    @endif
    <form action="{{ url('admin/edit/' . $entry['id']) }}" method="POST">
        @csrf
            ...
            <div class="form_birth01">
                <select class="form_birth_box" name="birthday_year" data-trigger-selectbox="select">
                    ...
                    @foreach($years as $key => $year)
                        <option value="{{ $key }}" {{ $key === (int) $data['birthday_year'] ? 'selected' : '' }}>{{ $year }}</option>
                    @endforeach
                </select>
            </div>  
            <div class="form_birth01">
                <select class="form_birth_box" name="birthday_month" data-trigger-selectbox="select">
                    ...
                    @foreach($months as $key => $month)
                        <option value="{{ $key }}" {{ $key === (int) $data["birthday_month"] ? "selected" : "" }}>{{ $month }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form_birth01">
                <select class="form_birth_box" name="birthday_day" data-trigger-selectbox="select">
                    ...
                    @foreach($days as $key => $day)
                        <option value="{{ $key }}" {{ $key === (int) $data["birthday_day"] ? "selected" : "" }}>{{ $day }}</option>
                    @endforeach
                </select>
            </div>
            @error('birthday_year')
